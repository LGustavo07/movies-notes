...
class UsersController {
    async create(request, response) {
        ...
        ); //inserir elementos do insomnia para o banco de dados.
    ...
    async update(request, response) {
        ...
        if (password && !old_password) {
            //verifica se usuário não deixou o campo vazio
            throw new appError(
        ...
        if (password && old_password) {
            //se os dois campos forem digitados
            const checkOldPassword = await compare(old_password, user.password);

            if (!checkOldPassword) {
                //se for falso a comparação, retorna a seguinte mensagem.
